...
import AmplifyApi from "../utils/AmplifyApi";
...
export default () => {
  ...
  useEffect(() => {
    const fetchDataQualityAnalyzers = async () => {
      const result = await AmplifyApi.getConstraint(analyzerType);
      setDataQualityAnalyzers(
          (await result).body.sort((a, b) => {
  ...
  async function createDataQualityAnalyzer() {
    ...
    const apiData = await AmplifyApi.createConstraint(analyzerType, newInput);
    setDataQualityAnalyzers([ ...dataQualityAnalyzers, apiData.body ]);
      ...
      {dataQualityAnalyzersToShow.map(dataQualityAnalyzer => (
        <Analyzer
          ...
          toggleEnable={async () => {
            ...
            const result = await AmplifyApi.updateConstraint(analyzerType, inputData)

            setDataQualityAnalyzers(
              dataQualityAnalyzersToShow.map(n => {
                return n.id === dataQualityAnalyzer.id ? result.body : n;
              })
            );
          }}
          onDelete={async () => {
            await AmplifyApi.deleteConstraint(analyzerType, dataQualityAnalyzer.id)
