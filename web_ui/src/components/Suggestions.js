...
export default () => {
  ...
  useEffect(() => {
    const fetchDataQualitySuggestions = async () => {
      const result = await AmplifyApi.getConstraint(suggestionType);
  ...
  async function createDataQualitySuggestion() {
    ...
    const apiData = await AmplifyApi.createConstraint(suggestionType, newInput);
    setDataQualitySuggestions([ ...dataQualitySuggestions, apiData.body ]);
      ...
      {dataQualitySuggestionsToShow.map(dataQualitySuggestion => (
        <Suggestion
          ...
          toggleEnable={async () => {
            ...
            const result = await AmplifyApi.updateConstraint(suggestionType, inputData)

            setDataQualitySuggestions(
              dataQualitySuggestionsToShow.map(n => {
                return n.id === dataQualitySuggestion.id ? result.body : n;
              })
            );
          }}
          onDelete={async () => {
             await AmplifyApi.deleteConstraint(suggestionType, dataQualitySuggestion.id)
